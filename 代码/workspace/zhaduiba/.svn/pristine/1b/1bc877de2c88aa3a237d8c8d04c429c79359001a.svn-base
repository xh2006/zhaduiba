		...
		function duierInfo_J2H(data) // 将堆儿信息json数据转换为HTML的div数据块
			{
				...
				$.each(jsondata.duierInfos,function(i, item) {
					duierInfo_html += '<div class=\"content_block\"><div class=\"content_block_title\"><div><a href=\"getDuierDetail?id='
						+ item.id
						+ '\">'
						+ item.name
						+ '</a></div><div><span>'
		...
		$(window).scroll(
				function() {
					...
					if ($(document).scrollTop() >= $(document).height()
						...
						$.post("getMoreDuierInfoByClassId_front",
							{id: classId, start : 0,items : 20},
							function(data){
								// 解析Json
...
<body>
	...
	<!--主要内容begin-->
	<input id="classId" type="hidden" name="field＿name" value="<s:property value='id' />"></input>
	<div id="content">
		<s:iterator value="duierShowInfoBlocks" var="block">
			<div class="content_block">
				...
				<div class="content_block_title">
					<div>
						<a href=""><s:property value="#block.duierName" /></a>
					</div>
					<div>
